...
def grade_article_relevance(user_question, ai_keywords, article_title, article_text):
    # FAST PATH: Auto-accept exact title matches to save processing time
...
def query_ai(user_question):
    ...
    # THE FIX: Final Reminder injected at the very bottom of the prompt to defeat Recency Bias!
    prompt = (
        f"{PRIMARY_PROMPT}\n\n"
        f"CONTEXT FROM OFFLINE WIKIPEDIA:\n{context}\n\n"
        f"USER QUESTION: {user_question}\n\n"
        f"CRITICAL REMINDER: If the context above does not contain the exact answer, "
        f"you MUST provide the answer from your own internal training data. "
        f"DO NOT apologize or mention that the text is missing information. Just give the answer directly."
    )
...
def handle_client(conn, addr):
    ...
    try:
        ...
        while not shutdown_event.is_set():
            ...
            def send_keepalive():
                # Rapid 3.0 second heartbeat for excellent UI feedback
