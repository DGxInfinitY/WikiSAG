...
2. Automatically download the massive ~52GB Wikipedia database directly from Kiwix (with resume support).
3. Optionally install itself as an automatic background `systemd` service.
...
Map the application to the TCP port you defined during the WikiSAG setup (default is `8000`).

```ini
APPLICATION 5,WIKI,C 5 HOST 0 S, ,WIKI,0
```
*(Ensure you have a TCP port configured in your linBPQ node that matches the routing above).*
...
If you ever need to change your AI model, update the max context tokens, or change the listening port, simply edit the `wikisag.ini` file located in your installation directory:
...
run_as_service = yes
```
If you modify `run_as_service`, simply run `wikisag` once in the terminal to allow the script to automatically install or remove the OS background service based on your changes.
